...
const CHOPFAST_API = (process.env.EXPO_PUBLIC_CHOPFAST_API_URL ?? '').replace(/\/$/, '');
const ANCHOR_MERCHANT =
  process.env.EXPO_PUBLIC_ANCHOR_MERCHANT_ID ?? '00000000-0000-0000-0000-000000000001';
...
export default function CartScreen() {
  ...
  const [appliedPromo, setAppliedPromo] = useState<PromoOk | null>(null);
  ...
  const itemSig = useMemo(() => items.map((i) => `${i.id}:${i.quantity}`).join('|'), [items]);
  useEffect(() => {
  ...
  const fetchRecommendations = async () => {
    ...
    const { data } = await supabase
      .from('menu_items')
      .select('id, name, price, image_url, category_id, merchant_id, is_available')
  ...
  const total = appliedPromo?.valid
    ? Number(appliedPromo.total)
    : subtotal + deliveryFee;

  const applyPromoCode = async () => {
    if (!promoCode.trim()) return;
    if (!CHOPFAST_API) {
      Alert.alert('Unavailable', 'Set EXPO_PUBLIC_CHOPFAST_API_URL to validate merchant promo codes.');
      return;
    }
    setIsApplying(true);
    const merchantId = items.find((i) => i.merchant_id)?.merchant_id ?? ANCHOR_MERCHANT;
    try {
      const res = await fetch(`${CHOPFAST_API}/api/v1/checkout/validate-promo`, {
        method: 'POST',
      ...
      if (!data?.valid) {
        ...
        setAppliedPromo({
          ...
          code: String(data.code ?? promoCode),
          discount: Number(data.discount ?? 0),
          delivery_fee: Number(data.delivery_fee ?? deliveryFee),
          total: Number(data.total ?? subtotal + deliveryFee),
        });
        Alert.alert('Success', `Code ${String(data.code ?? promoCode).toUpperCase()} applied.`);
      }
    } catch {
      Alert.alert('Error', 'Could not reach checkout service.');
      setAppliedPromo(null);
    }
    setIsApplying(false);
  };
  ...
  const handlePaymentSuccess = async (response: any) => {
    ...
    const merchantId = items.find((i) => i.merchant_id)?.merchant_id ?? ANCHOR_MERCHANT;
    const finalDelivery = appliedPromo?.valid ? appliedPromo.delivery_fee : deliveryFee;
    const finalDiscount = appliedPromo?.valid ? appliedPromo.discount : 0;
    const finalTotal = appliedPromo?.valid ? appliedPromo.total : subtotal + deliveryFee;
    ...
    const { data: order, error } = await supabase
      ...
      .insert({
        ...
        applied_promotion_id: appliedPromo?.valid ? appliedPromo.promotion_id : null,
        coupon_code: appliedPromo?.valid ? appliedPromo.code.toUpperCase() : null,
      })
      .select()
      .single();
    ...
    <ThemedView style={styles.container}>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.summaryContainer}>
          ...
          {appliedPromo?.valid && appliedPromo.discount > 0 && (
            <View style={styles.summaryRow}>
              <ThemedText style={[styles.summaryLabel, { color: Theme.colors.success }]}>Promo</ThemedText>
              <ThemedText style={[styles.summaryValue, { color: Theme.colors.success }]}>
